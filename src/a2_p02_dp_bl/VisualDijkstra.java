...
public class VisualDijkstra implements VisualizedGraphAlgo<GraphVertex, DefaultWeightedEdge> {

	GraphAlgoState<GraphVertex, DefaultWeightedEdge> m_graphState;
	GraphAlgoDijkstra m_algorithm;
	ImageIcon m_mapIcon;
	...
	public String getTitle(int subIndex) {
		...
		switch(subIndex) {
		case 0:
			title = "Dijkstra Graph3 : Husum -> Hamburg";
			break;
		case 1:
			title = "Dijkstra Graph3 : Minden  -> Hamburg";
			break;
		case 2:
			title = "Dijkstra Graph3 : Münster -> Hamburg";
			break;
		case 3:
			title = "Dijkstra Graph3 : Zufälliger Graph mit 20 Knoten";
			break;
		case 4:
			title = "Dijkstra Graph3 : Zufälliger Graph mit 30 Knoten";
			break;
		}
		return title;
	}
	...
	public GraphAlgoState<GraphVertex, DefaultWeightedEdge> init(int subIndex) {
		...
		m_algorithm = new GraphAlgoDijkstra(graph, m_graphState);
	...
	public void start() {
		...
		case 0:
			m_algorithm.search(new GraphVertex("Husum"), new GraphVertex("Hamburg"));
			break;
		case 1:
			m_algorithm.search(new GraphVertex("Minden"), new GraphVertex("Hamburg"));
			break;
		case 2:
			m_algorithm.search(new GraphVertex("Münster"), new GraphVertex("Hamburg"));
			break;
		default:
			m_algorithm.search(new GraphVertex("s"), new GraphVertex("z"));
			break;
		}			
	}
	...
	public VertexStyle getVertexStyle(GraphVertex vertex) {
		Integer g = m_graphState.getVertexProperty(vertex, GraphAlgoDijkstra.distancePropertyName);
		Boolean c = m_graphState.getVertexProperty(vertex, GraphAlgoDijkstra.closedPropertyName);
		Boolean o = m_graphState.getVertexProperty(vertex, GraphAlgoDijkstra.openPropertyName);
		Boolean p = m_graphState.getVertexProperty(vertex, GraphAlgoDijkstra.pathPropertyName);
		String style = "ASV";
	...
	public String getEdgeStyle(DefaultWeightedEdge edge) {
		...
		GraphVertex vsourcePredec = m_graphState.getVertexProperty(vsource, GraphAlgoDijkstra.predecessorPropertyName);
		GraphVertex vtargetPredec = m_graphState.getVertexProperty(vtarget, GraphAlgoDijkstra.predecessorPropertyName);
		
		Boolean srcPath = m_graphState.getVertexProperty(vsource, GraphAlgoDijkstra.pathPropertyName);
		Boolean dstPath = m_graphState.getVertexProperty(vtarget, GraphAlgoDijkstra.pathPropertyName);
